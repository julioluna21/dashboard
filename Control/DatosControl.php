      ...
      if (!empty($_FILES['miarchivo']['name']) && in_array($_FILES['miarchivo']['type'], $csvMimes)) {
        ...
        $dato10=str_replace("$",'',$dato10);    
            if($dato3!="" and $dato5!="" and $dato7!="" and $dato8!="" and $dato9!="" ){
			  ...
			  $fechaR = date('Y-m-d' ,strtotime($dato10));	  
              //$newDate = $arr[2].'-'.$arr[1].'-'.$arr[0];*/
			//$rspta = $Consulta->validar($contrato, $dato2,$dato10);	  
			 //if($rspta->num_rows<=0){
				 $data_values = array('ID_EJECUCION_CONTRATO'=>$contrato,"CATEGORIA"=>$dato2,'FEC_DET_EJE_PEAJE'=>$fechaR,'TOTAL_TRAIFICO'=>$dato3,'TOTAL_RECUDO'=>($dato3*$dato4),'TOTAL_TRAFICO_EXCLUSIVO'=>$dato5,'TOTAL_RECUDO_EXCLUSIVO'=>($dato5*$dato6),'TRAFICO_EXT_LEY'=>$dato7,'TRAFICO_EXT_CONSECION'=>($dato8+$dato9),'ESTADO_JECUCION'=>1);

    ...
    try {
      if (empty($idp)) {
		 ...
		 for($i=0;$i<count($datos);$i++){
			 $centro=limpiarCadena($datos[$i]['idcentro']);
			 $elemento=limpiarCadena($datos[$i]['idelemento']);
			 $cantidad=limpiarCadena($datos[$i]['cantidad']);
             $valorU=limpiarCadena($datos[$i]['valorU']);
			 $valorT=limpiarCadena($datos[$i]['valotT']);
			 $valorE=limpiarCadena($datos[$i]['valorejecutado']);
			 $data_values = array('ID_GESTION_PRS'=>$id,"ITEM_GESTION_GASTOS"=>$elemento,'CANTIDAD'=>$cantidad,'VALOR_UNITARIO'=>$valorU,"VALOR_TOTAL"=>$valorT,'VALOR_EJECUTADO'=>$valorE,'CENTRO_OPDETALLE_GESTION'=>$centro);
		...
		for($i=0;$i<count($datos);$i++){
			 ...
			 $valorT=limpiarCadena($datos[$i]['valotT']);
			 $valorE=limpiarCadena($datos[$i]['valorejecutado']);
			 $data_values = array('ID_GESTION_PRS'=>$idp,"ITEM_GESTION_GASTOS"=>$elemento,'CANTIDAD'=>$cantidad,'VALOR_UNITARIO'=>$valorU,"VALOR_TOTAL"=>$valorT,'VALOR_EJECUTADO'=>$valorE,'CENTRO_OPDETALLE_GESTION'=>$centro);
             $rspta = $GestionPresupuesto->insertar('detalle_gestion_presupuesto', $data_values);
	...
	case 'listarInicial':
      try {
		...
		if($regP->ESTADO_PRESUPUESTO==1){
		  ...
		  {
		  $valorElemento=str_replace(".", "",$reg3->VALOR_EJECUTADO);
		  $valorT=$valorT+$valorElemento;	  
			...
			$valp=number_format($valoP);		 
			if($valoP>=$valorT){
				$valp='<span style="color:green">$'.$valp.'</span>';	 
			}else{
				$valp='<span style="color:red">$'.$valp.'</span>';	
			}	 
		   ...
		   $reg=$rsptaV->fetch_object();	
			...
			$valp=number_format($reg->VALOR_PRESUPESUTO);		 
			if($reg->VALOR_PRESUPESUTO>=$reg->VALOR_TOATAL){
				$valp='<span style="color:green">$'.$valp.'</span>';	 
			}else{
				$valp='<span style="color:red">$'.$valp.'</span>';	
			}	 
      ...
      try {
		 ...
		 $bot='<SPAN title="Editar"><button class="btn btn-light" onclick="mostrarPresupuesto('.$rsptaP['ID_PRESUPUESTO'].')"><i class="fa fa-eye" style=""></i></button></SPAN>';  	
		  ...
		  $data[]=array(
			  ...
			  "3"=>"$".number_format($rsptaP['VALOR_PRESUPUESTO']),	
			  "4"=>"0",	
			  "5"=>$tiempocobro,	
			  "6"=>$tipopago,	
			  "7"=>"ABIERTA",	 
		  ...
		  {
		  $valorElemento=str_replace(".", "",$regG->VALOR_EJECUTADO);
		  $valorT=$valorT+$valorElemento;	  
		  }	  
		...
		$bot='<SPAN title="Editar"><button class="btn btn-light" onclick="mostrarGEstion('.$rsptaP['ID_PRESUPUESTO'].','.$anio.','.$mes.')"><i class="fa fa-eye" style=""></i></button></SPAN>'; 	
			...
			$valp=number_format($rsptaP['VALOR_TOTAL']);		 
			if($rsptaP['VALOR_TOTAL']>=$valorT){
				$valp='<span style="color:green">$'.$valp.'</span>';	 
			}else{
				$valp='<span style="color:red">$'.$valp.'</span>';	
			}	 
	...
	case 'listarCompra':
      ...
      case 'mostrarPresupuesto':
	   ...
	   case 'MostrarGestion'://activado por el ajax 
                   ...
                   $rsptaG = $Consulta->detallePresuestoG($idGestion);
			       $data= Array();

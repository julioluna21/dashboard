  ...
  case 'guardar':
    try {
      if (empty($id)){
		 ...
		 if($rspta){
		 $id=$rspta;	 
		  $data_values = array('IDContratoDetalle' =>$id,'NoOS'=>0,'FechaInicio'=>$fechai,'fechaFinal'=>$fechaf,'ObjetoContrato'=>$objeto,'ValorMensualContrato'=>$valorm,'ValorTotalContrato'=>$valort,'TipoTarifa'=>$tarifa);
    ...
    try {
		 ...
		 $rspta = $Consulta->conteootrosi($id);
		 $cantidad=$rspta["cantidad"];
		 $cantidad=$cantidad+1;
		 $data_values = array('IDCONTRATO_OTS'=>$id,'NOOS'=>$cantidad,'OBJETO'=>$objeto,'ESTADOOTROSI'=>1);
         $rspta = $Contrato->insertar('OTROSICONTRATO', $data_values); 
		 if($rspta){
		 $data_values = array('NoOS'=>$cantidad);
		 $where_condition = array('IDContratoDetalle' => $id);	
         $rspta = $Contrato->editar('detallecontrato', $data_values, $where_condition);
		 echo $rspta? "Registro exitoso": "Error no se actualizo el numeral del otro si";		
		 }else{
    ...
    case 'listar':
      try {
          ...
          {            
            $bot='<SPAN title="OTRO SI"><button class="btn btn-light" onclick="otsagregar('.$reg->IDcontrato.','.($reg->NoOS+1).')"><i class="fa fa-plus" style=""></i></button></SPAN> <SPAN title="Editar"><button class="btn btn-light" onclick="mostrar('.$reg->IDcontrato.')"><i class="fa fa-eye" style=""></i></button></SPAN>';

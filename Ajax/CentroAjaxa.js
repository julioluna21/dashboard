...
//al oprimir el boton del formulario
	...
	$.post("../Control/CentroOPControl.php?op=select", function(data)
    {
     $("#regional").html(data);
    });
...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/CentroOPControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/CentroOPControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(idcentro)
{
    ...
    $.post("../Control/CentroOPControl.php?op=mostrar",{idcentro : idcentro}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idcentro").val(data.IDCentroOP);
    $("#nombre").val(data.NombreCentroOP);             
    $('#regional').val(data.IDRegionalCentroOP).trigger('change');   
    $('#tipo').val(data.TipoCentroOP);   	 
...
function anular(idcentro){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/CentroOPControl.php?op=desactivar",{idcentro : idcentro}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(idcentro){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/CentroOPControl.php?op=activar",{idcentro : idcentro}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

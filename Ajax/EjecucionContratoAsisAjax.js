...
//al oprimir el boton del formulario
	...
	$.post("../Control/EjecucionContratoControl.php?op=select", function(data)
    {
     $("#contrato").html(data);
    });
	
	$.post("../Control/EjecucionContratoControl.php?op=select2", function(data)
    {
     $("#centroop").html(data);
    });
...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/ejecucionAsistencialesControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/ejecucionAsistencialesControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/ejecucionAsistencialesControl.php?op=mostrar",{idcontrato : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idcontrato").val(data.ID_EJE_CON_SER_ASIS);
    $('#contrato').val(data.ID_CONTRATO).trigger('change');             
    $('#centroop').val(data.ID_CENTROPEATIVO).trigger('change');   
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ejecucionAsistencialesControl.php?op=desactivar",{idcontrato : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ejecucionAsistencialesControl.php?op=activar",{idcontrato : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

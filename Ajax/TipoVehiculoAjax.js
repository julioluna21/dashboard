...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/TipoVehiculoControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/TipoVehiculoControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/TipoVehiculoControl.php?op=mostrar",{idtipo : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idtipo").val(data.ID_TIP_VEH);
    $("#nombre").val(data.NOM_TIPO_VEHICULO);              	 
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/TipoVehiculoControl.php?op=desactivar",{idtipo : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/TipoVehiculoControl.php?op=activar",{idtipo : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

var tabla,estado,fecha="",centro="",opciones="", centrosRporte=[];//variable global 
...
//al oprimir el boton del formulario
    $("#formregistros").on("submit",function(e)//e = variable que contiene el objeto
    {
     ...
     $("#formreporte").on("submit",function(e)//e = variable que contiene el objeto
    {
            Reporte(e);//guarda o edita el articulo
    });
...
$('#ReporteCentro').select2({
    width: '100%' ,
    dropdownParent: $('#modal-reporte .modal-body')
...
function Reporte(e){
    ...
    formData.append("detalleR", JSON.stringify(centrosRporte));
    $.ajax({
    url: "../Control/DatosAsistencialesControl.php?op=Reporte",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function agregartablaR(){
const fechaInicial=$("#fechainicalR").val();
const fechaFinal=$("#fechafinalR").val();
const Centro=$("#ReporteCentro").val();
var texto = $("#ReporteCentro option:selected").text();
centrosRporte.push({texto:Centro});
if(Centro!="" && fechaInicial!="" && fechaFinal!=""){
var html='<tr><tr><td>'+texto+'</td></tr><td><SPAN title="Cancelar elemto"><button class="btn btn-danger"style="margin:2px"  onclick=""><i class="fa fa-times"></i></button></SPAN></td></tr><tr>'
document.getElementById("tblreporte").insertRow(1).innerHTML=html;   
}else{
bootbox.alert("debe seleccionar las fechas realacionadas al reporte");  
}
}
...
$(document).on('click', '.btn-danger', function (event) {
    ...
    var Tabla1 = document.getElementById("tblreporte");
    const CeldasDeFila = Tabla1.rows[indice].getElementsByTagName('td');//datos necesarios para la tabla detalle salida
    const centro=CeldasDeFila[0].textContent.toLowerCase();
    let index = centrosRporte.findIndex(item => item.Centro === centro);
    if (index !== -1) {
    centrosRporte.splice(index, 1);
    }

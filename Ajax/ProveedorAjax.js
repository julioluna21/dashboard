...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/ProveedorControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/ProveedorControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/ProveedorControl.php?op=mostrar",{idproveedor : id}, function(data)
    {
	...
	$("#nit").val(data.NIT_PROVEEDOR);  		 
    $("#nombre").val(data.RAZON_SOCIAL); 
	$("#tel").val(data.TEL_PROVEEDOR);  
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ProveedorControl.php?op=desactivar",{idproveedor : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ProveedorControl.php?op=activar",{idproveedor : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

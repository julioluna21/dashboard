...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/SocioControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/SocioControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/SocioControl.php?op=mostrar",{idsocio : id}, function(data)
    {
    ...
    $("#nit").val(data.NitSocio);  
	$("#nombre").val(data.NombreSocio);  		 
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/SocioControl.php?op=desactivar",{idsocio : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/SocioControl.php?op=activar",{idsocio : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

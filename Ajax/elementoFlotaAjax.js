...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/ElementoControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/ElementoControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(idelemento)
{
    ...
    $.post("../Control/ElementoControl.php?op=mostrar",{idelemento : idelemento}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idelemento").val(data.IDELEMETO);
    $("#nombre").val(data.NOMBRE_ELEMENTO);   			 
    $("#descripcion").val(data.DESCRIPCION);              
    $imgPreview = document.querySelector("#imagen");
	$imgPreview.src = data.IMAGEN_REFERENCIA; 	 
...
function anular(idelemento){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ElementoControl.php?op=desactivar",{idelemento : idelemento}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(idelemento){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ElementoControl.php?op=activar",{idelemento : idelemento}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/TipoServicioAsistencialControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/TipoServicioAsistencialControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/TipoServicioAsistencialControl.php?op=mostrar",{idservicio : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idservicio").val(data.ID_TIPO_EVENTO);
    $("#nombre").val(data.NOMBRE_TIPO_EVENTO);  		              
    }, 1000);      
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/TipoServicioAsistencialControl.php?op=desactivar",{idservicio : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/TipoServicioAsistencialControl.php?op=activar",{idservicio : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

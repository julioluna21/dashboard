...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/EjecucionContratoControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/EjecucionContratoControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/EjecucionContratoControl.php?op=mostrar",{idcontrato : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idcontrato").val(data.IDEjecucion);
    $('#contrato').val(data.IdContratoEjecuCicion).trigger('change');             
    $('#centroop').val(data.IDCentroOPEjecucion).trigger('change');   
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/EjecucionContratoControl.php?op=desactivar",{idcontrato : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/EjecucionContratoControl.php?op=activar",{idcontrato : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

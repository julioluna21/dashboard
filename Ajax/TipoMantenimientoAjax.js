...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/TipoMantenimentoControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/TipoMantenimentoControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/TipoMantenimentoControl.php?op=mostrar",{idtipo : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idtipo").val(data.ID_TIPO_MNT);
    $("#nombre").val(data.NOMBRE_TIPO_MNT);              	 
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/TipoMantenimentoControl.php?op=desactivar",{idtipo : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/TipoMantenimentoControl.php?op=activar",{idtipo : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

...
{
    limpiar();
	$('#tipovh').val("").trigger('change');
	$imgPreview = document.querySelector("#imagen");
	$imgPreview.src = "";
    mostrarform(false);
}
...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/VehiculoControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/VehiculoControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/VehiculoControl.php?op=mostrar",{idvehiculo : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idvehiculo").val(data.ID_VEH);
    $("#placa").val(data.PLACA_VEH);  
	$("#marca").val(data.MARCA_VEH);  
	$("#modelo").val(data.MODELO_VEH);  
	$("#propietario").val(data.PROPIETARIO_VEH);  
	$("#comodato").val(data.TIPO_COMODATO);  		 
    $('#tipovh').val(data.TIPO_VEH).trigger('change'); 
	$imgPreview = document.querySelector("#imagen");
	$imgPreview.src = data.IMG_VEHICULO;		 
    }, 1000);      
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/VehiculoControl.php?op=desactivar",{idvehiculo : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/VehiculoControl.php?op=activar",{idvehiculo : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

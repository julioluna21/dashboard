...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/ProyectoControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function mostrar(id)
{
    ...
    $.post("../Control/ProyectoControl.php?op=mostrar",{idproyecto : id}, function(data)
    {
	...
	$("#nombre").val(data.nombreProyecto);  
	$('#contrato').val(data.IDcontratoProyecto).trigger('change');		 
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ProyectoControl.php?op=desactivar",{idproyecto : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ProyectoControl.php?op=activar",{idproyecto : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

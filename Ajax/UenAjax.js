...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/UenControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/UenControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/UenControl.php?op=mostrar",{iduen : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#iduen").val(data.IDUen);
    $("#nombre").val(data.NombreUen);              	 
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/UenControl.php?op=desactivar",{iduen : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/UenControl.php?op=activar",{iduen : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

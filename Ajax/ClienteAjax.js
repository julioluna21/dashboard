...
//al oprimir el boton del formulario
	...
	$.post("../Control/ClienteControl.php?op=socios",function(data){
        $("#mdacceso").html(data);
    });
	
	 $.post("../Control/ClienteControl.php?op=select", function(data)
    {
     $("#tipo").html(data);	 
    })
...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/ClienteControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/ClienteControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(id)
{
    ...
    $.post("../Control/ClienteControl.php?op=mostrar",{idcliente : id}, function(data)
    {
    ...
    mostrarform(true)       
    $("#idcliente").val(data.dato1.IDClinente);
    $("#nit").val(data.dato1.NitCliente);  
	$("#nombre").val(data.dato1.NombreCliente);  
	$('#tipo').val(data.dato1.TipoCliente).trigger('change');	
	$("#administrador").val(data.dato1.AdministradorCliente); 
	socios=data.dato2;	
	socios.forEach(function(da) {		 
    $("#r"+da.IDSocioU).prop("checked", true);  
    });			 
...
function anular(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ClienteControl.php?op=desactivar",{idcliente : id}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(id){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ClienteControl.php?op=activar",{idcliente : id}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,

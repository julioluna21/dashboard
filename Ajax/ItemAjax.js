...
function listar(estado)
{
    tabla=$('#tbllistado').dataTable(//Carga variable con datos datatable
    {
      ...
      paginate: {
        ...
        "ajax"://metodo ajax
                            {
                                    url: '../Control/ItemControl.php?op=listar&estado='+estado,//pagina que realiza la operación
...
function guardar(e)
{
    ...
    $.ajax({
            url: "../Control/ItemControl.php?op=guardar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function mostrar(iditem)
{
    ...
    $.post("../Control/ItemControl.php?op=mostrar",{iditem : iditem}, function(data)
    {
...
function anular(iditem){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ItemControl.php?op=desactivar",{iditem : iditem}, function(data)
            {
      bootbox.alert({
                        title: 'Desactivado!',
                        message: data,
...
function activar(iditem){
     ...
     bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                  $.post("../Control/ItemControl.php?op=activar",{iditem : iditem}, function(data)
            {
      bootbox.alert({
                        title: 'Activado!',
                        message: data,
